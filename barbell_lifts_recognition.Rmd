...
# load the libraries for learning functions
suppressMessages({
...
### Reading the Data
The data sets are available on the course project site in two files:
the [first one][2] containing the training observation already classified;
the [second one][3] is the testing data, which should be classified with the 
constructed model.
...
At first, let's exclude the variables, which are not defined (have all `NA` 
values) in the testing data set, so they could not be used in the model to 
predict an outcome.
...
We will also exclude the variables with *zero* or *near zero variance*, because they do not give strong evidence towards a certain classification outcome.
...
```{r predictors4, cache=TRUE,fig.width=8}
featurePlot(x=train.data[, vars[grep('^(magnet|yaw)', vars)]],
            y= train.data$classe, 'strip', jitter=TRUE)

featurePlot(x=train.data[, vars[grep('^(gyros|roll)', vars)]],
            y= train.data$classe, 'strip', jitter=TRUE)

featurePlot(x=train.data[, vars[grep('^(accel|pitch)', vars)]],
            y= train.data$classe, 'strip', jitter=TRUE)
```
...
fit1 <- train(f, data=train.data, method='rf', ntree=1); fit1$finalModel$confusion
fit5 <- train(f, data=train.data, method='rf', ntree=5); fit5$finalModel$confusion
fit10 <- train(f, data=train.data, method='rf', ntree=10); fit10$finalModel$confusion
fit20 <- train(f, data=train.data, method='rf', ntree=20); fit20$finalModel$confusion
```
...
models yields `r round(fit5$finalModel$err.rate[5,1]*100,2)`% OOB error rate.
The 10 and 20-tree models yield `r round(fit10$finalModel$err.rate[10,1]*100,2)`%
and `r round(fit20$finalModel$err.rate[20,1]*100,2)`% OBB error rates
accordingly.
